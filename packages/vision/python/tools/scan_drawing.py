...
The agent sees: "Here are N types of symbols on this page, with counts and thumbnails."
No zooming, no manual bbox hunting.

Usage:
    # Python API
    from tools.scan_drawing import scan_page
    inventory = scan_page("/path/to.pdf", page=1)

    # CLI (stdin JSON)
    echo '{"pdfPath": "/path/to.pdf", "pageNumber": 1}' | python3 tools/scan_drawing.py
"""
...
try:
    ...
    from find_symbols import find_symbol_candidates, validate_bbox_has_content, crop_component
    from count_symbols import count_matches
...
def pick_representative(cluster: list[dict], img: np.ndarray, iw: int, ih: int) -> dict | None:
    ...
    for c in cluster:
        ...
        cx_norm = abs(c["cx"] / iw - 0.5)
        cy_norm = abs(c["cy"] / ih - 0.5)
        center_score = 1.0 - (cx_norm + cy_norm)
...
def find_composite_candidates(img: np.ndarray, iw: int, ih: int,
    ...
    Catches nozzle flanges, equipment details, etc. that are multi-stroke symbols.
    """
    if len(img.shape) > 2:
    ...
    for i in range(1, nlabels):
        ...
        if x > iw * 0.7 and y > ih * 0.8:
            continue
        if x < 10 or y < 10 or x + w > iw - 10 or y + h > ih - 10:
            continue
...
def scan_page(pdf_path: str, page: int = 1, dpi: int = 150,
              min_size: int = 20, max_size: int = 180,
              min_cluster_size: int = 2,
              max_clusters: int = 10,
              count_threshold: float = 0.78) -> dict:
    """
    ...
    Returns:
        {
            clusters: [{
                id, representativeBox, thumbnail, sizeCategory,
                avgDimensions, candidateCount, matchCount,
                avgConfidence, topMatches, countDuration_ms,
            }],
            imageWidth, imageHeight, totalClusters, totalSymbolsFound,
            scanDuration_ms,
        }
    """
    start = time.time()
    ...
    # Pass A: connected components (small to medium symbols)
    cc_candidates = find_symbol_candidates(img, iw, ih,
                                            min_size=min_size, max_size=max_size,
                                            min_area=80, exclude_borders=False,
                                            border_margin=30)

    # Pass B: morphological dilation (larger multi-stroke composite symbols)
...
# CLI mode — reads JSON from stdin, writes JSON to stdout (same pattern as count_symbols.py)
if __name__ == "__main__":
    ...
    result = scan_page(
        ...
        max_size=payload.get("maxSize", 180),
    )

    # Strip thumbnails from CLI output to keep it manageable (same as count_symbols.py)
