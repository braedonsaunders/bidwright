...
2. Call \`buildDrawingAtlas\` once. It renders drawing pages, builds the sheet registry, queues optional Gemini drawing extraction when enabled, and creates searchable semantic regions from immediately available Azure/local/PDF-native evidence. Completed cached Gemini regions can be reused when present.
...
9. If queued Gemini extraction finishes after the current atlas build, call \`buildDrawingAtlas({ force: true })\`, search the newly available regions, and incorporate changed evidence without duplicating existing work.
...
- **addSourceToDrawingAtlas** â€” Agentically request a PDF/DWG/DXF source be added to the atlas with a rationale; queues Gemini drawing extraction for PDFs when enabled.
...
8. If queued Gemini extraction completes after the atlas build, rebuild/search the atlas and incorporate the new evidence without duplicating existing work.
