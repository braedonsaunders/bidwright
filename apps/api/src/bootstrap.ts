...
export async function applyPendingMigrations(): Promise<{ applied: boolean; reason?: string }> {
  ...
  // a sanity cap.
  // Bumped to 100 because the recovery is bounded by the total number of
  // migrations in prisma/migrations (45 today, growing). The same-name-
  // twice check below is the real guard against genuine progress blocks;
  // this cap only protects against pathological loops.
