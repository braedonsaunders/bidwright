...
export function ClientsList({
  ...
  async function handleCreateSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setCreateError(error instanceof Error ? error.message : "Could not create client.");
    } finally {
  ...
  const filters: Array<{ key: QuickFilter; label: string; count: number }> = [
    ...
    { key: "active", label: "Active pipeline", count: rows.filter((row) => row.active && row.metrics.activeCount > 0).length },
    { key: "quoted", label: "Quoted", count: rows.filter((row) => row.metrics.quoteCount > 0).length },
    { key: "prospects", label: "Prospects", count: rows.filter((row) => row.metrics.quoteCount === 0).length },
    { key: "inactive", label: "Inactive", count: rows.filter((row) => !row.active).length },
  ];
      ...
      <FadeIn className="shrink-0">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-fg/50">Accounts, quote history, pipeline health, and client value.</p>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative min-w-[280px] flex-1 max-w-xl">
            ...
            <Input
              ...
              placeholder="Search clients, contacts, quote titles, cities..."
              value={search}
          ...
          <span className="ml-auto text-[11px] text-fg/30 tabular-nums">
            {filtered.length === rows.length ? `${filtered.length} clients` : `${filtered.length} of ${rows.length} clients`}
          </span>
        </div>
      </FadeIn>
            ...
            <table className="w-full table-fixed text-sm">
              <thead className="sticky top-0 z-10 bg-panel">
                <tr className="border-b border-line">
                  <SortHeader label="Client" sortKey="name" activeKey={sortKey} sortDir={sortDir} onSort={handleSort} className="w-[55%] sm:w-[34%] lg:w-[24%]" />
                  <th className="hidden w-[20%] px-4 py-2.5 text-left text-[11px] font-medium uppercase tracking-wider text-fg/40 lg:table-cell">Contact</th>
                  <SortHeader label="Quotes" sortKey="quotes" activeKey={sortKey} sortDir={sortDir} onSort={handleSort} className="w-[17%] px-2 text-right sm:w-[9%] sm:px-4" />
                  <SortHeader label="Active" sortKey="activeValue" activeKey={sortKey} sortDir={sortDir} onSort={handleSort} className="w-[28%] px-2 text-right sm:w-[11%] sm:px-4" />
                  <SortHeader label="Awarded" sortKey="wonValue" activeKey={sortKey} sortDir={sortDir} onSort={handleSort} className="hidden w-[11%] text-right xl:table-cell" />
                  <SortHeader label="Win" sortKey="winRate" activeKey={sortKey} sortDir={sortDir} onSort={handleSort} className="hidden w-[7%] text-right 2xl:table-cell" />
                  <SortHeader label="Margin" sortKey="margin" activeKey={sortKey} sortDir={sortDir} onSort={handleSort} className="hidden w-[8%] text-right lg:table-cell" />
                  <SortHeader label="Updated" sortKey="updated" activeKey={sortKey} sortDir={sortDir} onSort={handleSort} className="hidden w-[10%] md:table-cell" />
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 && (
                      ...
                      <Link href={clientHref(row)} className="group flex min-w-0 items-center gap-3">
                        ...
                        <span className="min-w-0">
                          <span className="flex min-w-0 items-center gap-1.5">
                            ...
                            {!row.active && <Badge className="hidden shrink-0 text-[9px] sm:inline-flex">Inactive</Badge>}
                          </span>
                          <span className="mt-0.5 flex items-center gap-1.5 text-[11px] text-fg/40">
                            <MapPin className="h-3 w-3" />
                            <span className="truncate">{row.location || "No location"}</span>
                          </span>
                        </span>
                      </Link>
                    </td>
        ...
        <form onSubmit={handleCreateSubmit} className="rounded-lg border border-line bg-panel shadow-2xl">
          <div className="flex items-start justify-between gap-4 border-b border-line px-5 py-4">
            <div>
              ...
              <p className="mt-0.5 text-xs text-fg/50">Create the account record before quoting the work.</p>
            </div>
            <button
              type="button"
